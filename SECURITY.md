...
- **Email confirmation is on.** New email/password accounts must confirm their address before contributing. This reduces casual account abuse but does not replace rate limiting or moderation.
- **Self-service account deletion** removes the Supabase Auth user, profile, Community Notes, verifications, and reports. Factual place records and their photographs remain as community knowledge, with user attribution set to null (migration `0026`).
- **No self-delete capability for a place's own author.** If a place needs removing (spam, a genuine mistake), the author can only "Report" it, then it requires an admin (`profiles.is_admin`, `0016_admin_role.sql`) or the maintainer via the Supabase dashboard. See `docs/08-mvp.md`'s "Out of scope for V1" - a moderation dashboard and automated takedown were both explicitly deferred. Community notes are the one deliberate exception (D-020, migration `0022`): an author can delete their own note directly, since notes are low-stakes free text and mistakes are common - someone else's note can still only be reported (migration `0021`), not deleted, same manual-review model as everything else.
