...
- **No self-delete capability for any author, of a place or a community note.** If something needs removing (spam, a genuine mistake), the author can report it (place: "Report"; note: the per-note "Report" action added in migration `0021`) but not remove it themselves - it then requires an admin (`profiles.is_admin`, `0016_admin_role.sql`) or the maintainer via the Supabase dashboard. See `docs/08-mvp.md`'s "Out of scope for V1" - a moderation dashboard and automated takedown were both explicitly deferred.
